...
// This is nullptr if we are not using ESP32 timer interrupts.
hw_timer_t *MotionActuator::_isrMotionTimer;

// Static reference to a single MotionActuator instance
...
void IRAM_ATTR MotionActuator::setupNewBlock(MotionBlock *pBlock)
{
    ...
    for (int axisIdx = 0; axisIdx < RobotConsts::MAX_AXES; axisIdx++)
    {
        ...
        } else if (pAxisInfo->_adafruit_stepper)
        {
            // Even though we are not setting the direction pin here, set _totalStepsInc so we can use it later
...
bool IRAM_ATTR MotionActuator::handleStepMotion(MotionBlock *pBlock)
{
    ...
    if (_curStepCount[axisIdxMaxSteps] < _stepsTotalAbs[axisIdxMaxSteps])
    {
        ...
        if (pAxisInfo->_pinStep != -1)
        {
            ...
            adafruitOneStep(axisIdxMaxSteps);
        }
...
void IRAM_ATTR MotionActuator::_isrStepperMotion(void) {
    executeStepperMotion();
}

// Returns true if any axis moved.
bool IRAM_ATTR MotionActuator::executeStepperMotion(void)
{
...
// Returns true if it caused an axis movement
bool MotionActuator::process()
{
    bool moved = false;
    // If not using ISR call executeStepperMotion on every process call
    if (_isrMotionTimer == nullptr) {
        moved = executeStepperMotion();
    }
...
void MotionActuator::adafruitOneStep(int axisIdx) {
    RobotConsts::RawMotionAxis_t *pAxisInfo = &_rawMotionHwInfo._axis[axisIdx];
    bool forwards = (_totalStepsInc[axisIdx] > 0);
    pAxisInfo->_adafruit_stepper->onestep(forwards ? FORWARD : BACKWARD, INTERLEAVE);
}

void MotionActuator::enableInterrupts() {    
