...
constexpr int kMaxActuatorProcessCalls = 100;
...
void MotionHelper::configure(const char *robotConfigJSON)
{
    ...
    // Enable interrupts for the motion actuator unless any axis is using an
    // adafruit stepper, as the Adafruit library is not IRAM enabled.
    bool has_adafruit_stepper = false;
    for (int axisIdx = 0; axisIdx < RobotConsts::MAX_AXES; axisIdx++)
    {
        if (rawMotionHwInfo._axis[axisIdx]._adafruit_stepper) {
            has_adafruit_stepper = true;
            break;
        }
    }
    if (!has_adafruit_stepper)
        _motionActuator.enableInterrupts();
...
void MotionHelper::service()
{
    ...
    // Call process on motion actuator.
    // When interrupts are enabled, this returns immediately.
    // When interrupts are disabled, this may execute movement, and if so we try again
    // up to some reasonable limit. If you don't do this, you severely cap the maximum speed.
    for (int i = 0; i < kMaxActuatorProcessCalls; ++i)
    {
        // if (!_motionActuator.process()) break;
